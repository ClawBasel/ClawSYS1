...
client.on("ready", () => {
  ...
  client.user.setActivity(`Claw System`, { type: "Streaming" });
});
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@532269812190150657>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on('ready', () => {
    ...
    client.user.setActivity(`Claw Music 2`,{type: 'Streaming'}); ///التعديل علي البلاينج
});
client.on('message', async msg => {
    if(msg.author.bot) return undefined;
  if(!msg.content.startsWith(prefix)) return undefined;
...
const command = args.shift().toLowerCase();
    ...
    if(cmd === 'play') {
        ...
        if(!voiceChannel) return msg.channel.send(`<a:x_:747846340343627796> You must be listening in a voice channel to use that!`);
        const permissions = voiceChannel.permissionsFor(msg.client.user);
        if(!permissions.has('CONNECT')) {
            return msg.channel.send(`<a:x_:747846340343627796> I can't join Your voiceChannel because i don't have ` + '`' + '`CONNECT`' + '`' + ` permission!`);
        }
        ...
        if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			...
			try {
				...
				try {
					...
					console.error(err);
					return msg.channel.send('I can\'t find any thing<a:x_:747846340343627796> ');
				}
			}
...
// 14
      ...
      let queue = active.get(msg.guild.id);
        ...
        if(!vCh || vCh !== msg.guild.me.voiceChannel) return msg.channel.send(`You are not in my voice channel<a:x_:747846340343627796>.`);

        if(!queue) {
            return msg.channel.send('No music playing to pause<a:x_:747846340343627796>.')
        }
        ...
        disp.pause('Pausing.. <a:right:747846344885928007>')
...
// 2002
      ...
      let queue = active.get(msg.guild.id);
        ...
        if(!queue) return msg.channel.send(':notes: No music paused to resume<a:x_:747846340343627796>.')

        if(queue.playing) return msg.channel.send(':notes: No music paused to resume<a:x_:747846340343627796>.')
        ...
        msg.channel.send(':notes: Resumed<a:right:747846344885928007> .')

    } else if(cmd === 'volume') {

      let queue = active.get(msg.guild.id);

      if(!queue || !queue.songs) return msg.channel.send(':notes: There is no music playing to set volume <a:x_:747846340343627796>.');

      let vCh = msg.member.voiceChannel;

      if(!vCh || vCh !== msg.guild.me.voiceChannel) return msg.channel.send(':notes: You are not in my voice channel<a:x_:747846340343627796>.');

      let disp = queue.connection.dispatcher;

      if(isNaN(args[0])) return msg.channel.send(':notes: Numbers only!');

      if(parseInt(args[0]) > 100) return msg.channel.send('You can\'t set the volume more than 100 <a:x_:747846340343627796>.')
...
// 14-04-2002
      ...
      if(!queue.songs || queue.songs < 2) return msg.channel.send('There is no music to skip to <a:x_:747846340343627796>.');
...
// 14-04-2002
      ...
      if(!queue.songs[sN]) return msg.channel.send('There is no song with this number <a:x_:747846340343627796>.' );
...
client.on('ready', () => {
   console.log(`----------------`);
    ...
    console.log(`----------------`);
  console.log(`Logged in as ${client.user.tag}!`);
client.user.setGame(`System V2 `,"http://twitch.tv/S-F") //باسم سيرفرك او اي شي تبيه hello تعديل مهم عدل كلمة 
client.user.setStatus("dnd")
});
//// مهم
